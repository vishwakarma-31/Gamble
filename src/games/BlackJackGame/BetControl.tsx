...
export function BetControls({ balance, btnState, currentBet, onBetChange, onBet }: BetControlsProps) {
  ...
  const isBetButtonDisabled = btnState === "player-turn" || (btnState === "betting" && (currentBet <= 0 || currentBet > balance));
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Bet / End Game Button */}
      <button
        onClick={btnState === "game-over" ? onBet : onBet} // Call the same function for both scenarios
        disabled={isBetButtonDisabled} // Controlled by game state and current bet
