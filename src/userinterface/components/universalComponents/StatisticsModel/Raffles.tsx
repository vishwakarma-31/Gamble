...
const Raffles = () => {
    ...
    const FundData: FundDataType = {
        "Raffle Name": [
            { amount: 10, time: "Weekly", icon: CgDollar },
            { amount: 20, time: "Monthly", icon: CgDollar },
            { amount: 30, time: "Yearly", icon: CgDollar },
            { amount: 40, time: "Daily", icon: CgDollar },
        ],
        Date: [
        ...
        <div className="flex flex-col justify-between flex-shrink-0 mt-5 sm:mt-0 items-start w-full gap-4 overflow-x-auto">
            <div className="flex w-full min-w-[500px]">
                {paginatedItems.map(({ key, items }, index) => (
                    <div
                        key={key}
                        className="flex-1 flex w-full flex-col [&>div:nth-child(odd)]:bg-[#203044] [&>div:nth-child(even)]:bg-[#2d485b]"
                    >
                        {/* Column Header */}
                        <div
                            className={`flex ${
                                ...
                                {isLoading ? (
                                    ...
                                    >
                                        {key === "Raffle Name" && (
                                            <>
                                                <div>
                                                    {React.createElement((item as RaffleItems).icon, { size: 15 })}
                                                </div>
                                                <div>{(item as RaffleItems).amount}k</div>
                                                <div>{(item as RaffleItems).time} Raffles</div>
                                            </>
                                        )}
                                        {key === "Date" && <div className="pl-5">{(item as DateItems).date}</div>}
                                        {key === "Tickets" && <div>{(item as TicketItems).count}</div>}
                                    </div>
                                )}
                            </>
                        ))}
