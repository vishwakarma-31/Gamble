...
const LocalCurrencyWithdrawal=()=>{
     ...
     const ITEMS_PER_PAGE = 15; // Number of items per page
        ...
        const FundData = {
            Details: [
                { status: "Fail", time: "1:00 AM", date: "30/12/24", amount: 1000, transactionId: "XCTYUIJ#8765456",  },
                { status: "Success", time: "1:00 AM", date: "30/12/24", amount: 1000, transactionId: "XCTYUIJ#8765456",  },
                { status: "Success", time: "1:00 AM", date: "30/12/24", amount: 1000, transactionId: "XCTYUIJ#8765456",  },
                { status: "Fail", time: "1:00 AM", date: "30/12/24", amount: 1000, transactionId: "XCTYUIJ#8765456",  },
                { status: "Success", time: "1:00 AM", date: "30/12/24", amount: 1000, transactionId: "XCTYUIJ#8765456",  },
                { status: "Success", time: "1:00 AM", date: "30/12/24", amount: 1000, transactionId: "XCTYUIJ#8765456",  },
                ...
                <div className="flex w-full px-5 mt-5 py-5 rounded-md bg-[#0f1c2bda]">
                    ...
                    <div className="flex-1 w-full grid grid-rows-2 sm:gap-x-4 md:gap-0 text-[12px] md:text-[14px] [&>div:nth-child(odd)]:bg-[#2d485b]">
                        {currentItems.map((item, index) => (
                            <div
                                key={index}
                                className="py-4 flex justify-between rounded-md items-center flex-row gap-1 font-bold text-blue-50 px-4"
                            >
                                <div className="flex items-center space-x-1 flex-row">
                                    <VscCloudDownload size={18} />
                                    <div className="flex flex-col">
                                        <div>{item.status}</div>
                                        <div className="text-gray-400 font-semibold">Withdrawal</div>
                                        <div className="text-gray-400 font-semibold">
                                            {item.time} {item.date}
                                        </div>
                                    </div>
                                </div>
                                <div className="flex text-gray-400 justify-center items-end font-semibold flex-col">
                                    <div className="flex-row justify-center space-x-1 items-center flex">
                                        <div>{item.amount}</div>
                                        <div>
...
export default LocalCurrencyWithdrawal;