    ...
    return (
        <div className="w-full flex relative flex-col max-h-screen h-[63.5%] justify-between items-start">
            <div className="w-full h-full">
                ...
                <div onClick={handleOutsideClick} onMouseLeave={() => setDialogTextColor(false)} onMouseEnter={() => setDialogTextColor(true)} className="w-full pb-2 relative px-2 z-10 justify-between flex flex-column">
                    <div onClick={(e) => { setOddDialog(prev => !prev), e.stopPropagation() }} className={`text-[15px] flex cursor-pointer flex-column items-center text-[${!DialogTextColor ? "#afb6c1" : "white"}] font-medium`}>{DialogText}<IoIosArrowDown className="ml-2" /></div>
...
<div className="w-full flex flex-col">
  ...
  >
    ...
    <div className="flex items-center justify-center h-full w-14 sm:w-16">
      <LuTicketPercent size={24} className="text-gray-500" />
    </div>
    {/* Shimmer Text Lines */}
